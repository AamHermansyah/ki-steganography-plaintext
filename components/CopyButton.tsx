...
function CopyButton({ className, text }: PropTypes) {
  const copyToClipboard = () => {
    ...
    navigator.clipboard.writeText(text)
      ...
      .catch(() => {
        toast.error('Gagal menyalin teks. Coba lagi!');
      });
  };

  return (
    <button type="button" onClick={copyToClipboard} className={cn(
      'p-1 hover:bg-muted rounded-sm transition-all',
      className
    )}>
